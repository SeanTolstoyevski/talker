...
// For example : talker.Load(); talker.Output("Hello, I am Golang and CGO or Talker :)", false);  
...
// Convert UTF16 (wchar_t) c to GoString. You don't need to use it. Talker convert the strings you send to suitable formats.
func WCharToString(p *C.wchar_t) string {
	return windows.UTF16PtrToString((*uint16)(p))
}

// Convert go string  c to wchar_t. You don't need to use it.
